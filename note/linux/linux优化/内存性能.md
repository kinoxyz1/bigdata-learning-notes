...
- hcache: [下载地址](https://github.com/silenceshell/hcache/archive/refs/tags/v1.0.0.tar.gz)
- vmtouch: `apt install vmtouch`

### 2.3.1 hcache
hcache 可以查看cache的使用情况, 比如列出topN, 列出指定进程的cache使用情况

### 2.3.2 vmtouch
统计出某个文件、目录，甚至是嵌套目录下各文件真正使用cache的大小，从而明确cache消耗的分布情况。
